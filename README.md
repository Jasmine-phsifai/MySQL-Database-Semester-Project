...
2. **首次** 双击 `install.bat` 自动用清华源装齐 Python 依赖 (已装则跳过)
3. 双击 `start.bat`
4. 登录: `admin / admin123` (或 `editor / demo123` / `viewer / demo123`)

## 环境配置 (一键)

`install.bat` 自动按以下顺序检测 / 安装:

1. 选解释器: `runtime\python.exe` → `D:\Anaconda\envs\py312\python.exe` → PATH 中的 `python`
2. 升级 pip (清华镜像)
3. 检测 8 个核心包 — 已装则跳过, 缺失列出
4. 用 `https://pypi.tuna.tsinghua.edu.cn/simple` 装 `requirements.txt`
5. 复检全部能 import
...
| 命令 | 用途 |
|------|------|
| `install.bat`                  | 自动选最佳解释器 (默认) |
| `install.bat -Target conda`    | 强制装到 D:\Anaconda\envs\py312 |
| `install.bat -Target system`   | 装到 PATH 上的 python |
| `install.bat -Target runtime`  | 装到 runtime\python.exe (打包用) |
| `install.bat -Force`           | 全部包重装一遍 |

`start.bat` 启动时会自检依赖, 缺失会要求你先跑 `install.bat`.
...
:: 3. (可选) 提取真实课程数据 (PDF -> CSV)
"D:\Anaconda\envs\py312\python.exe" tools\extract_courses_pdf.py
"D:\Anaconda\envs\py312\python.exe" tools\import_md_table.py
...
| 启动后提示缺包  | 双击 `install.bat` (清华镜像一键装) |
